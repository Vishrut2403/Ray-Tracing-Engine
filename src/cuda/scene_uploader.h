...
private:
	...
	static GpuScene build_sss(const std::string& name) {
		...
		int white_id = add_lambertian(mats, vec3(0.73f,0.73f,0.73f));
		int green_id = add_lambertian(mats, vec3(0.12f,0.45f,0.15f));
		int light_id = add_light(mats, vec3(60.f, 56.f, 50.f));
		...
		// SSS spheres - reduced size for better light penetration
		// Skin — warm salmon, short mfp, ior typical for biological tissue
		int skin_id = add_sss(mats,
			vec3(0.85f, 0.40f, 0.25f), // albedo
			0.08f,                      // mfp (8% of box scale)
			1.4f);                      // ior
		add_sphere(hits, vec3(160, 100, 180), 60.f, skin_id);

		// Marble — near-white, long mfp, very translucent
		int marble_id = add_sss(mats,
			vec3(0.92f, 0.89f, 0.82f), // albedo
			0.20f,                      // mfp
			1.3f);                      // ior
		add_sphere(hits, vec3(390, 120, 350), 70.f, marble_id);

		// Jade — deep green, moderate mfp for better visibility
		int jade_id = add_sss(mats,
			vec3(0.18f, 0.55f, 0.34f), // albedo
			0.10f,                      // mfp (increased from 0.03 for visibility)
			1.5f);                      // ior
		add_sphere(hits, vec3(390, 310, 120), 60.f, jade_id);
