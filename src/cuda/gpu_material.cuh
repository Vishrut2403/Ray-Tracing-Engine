...
constexpr real GPU_PI = 3.1415926535897932385;
...
// GGX helpers
__device__ inline real ggx_D(real ndoth, real a) {
	// a2*c2 + (1-c2), not c2*(a2-1)+1: in single precision the latter cancels
	// to exactly zero near normal incidence once a2 falls below the epsilon.
	real a2 = a*a, c2 = ndoth*ndoth;
	real d  = a2*c2 + rmax(1.0 - c2, 0.0);
	return a2 / (GPU_PI * d * d);
}

__device__ inline real ggx_G1(real ndotv, real a) {
	real a2 = a*a;
	return 2.0*ndotv / (ndotv + sqrt(a2 + (1.0-a2)*ndotv*ndotv));
}

__device__ inline real ggx_G2(real ndotv, real ndotl, real a) {
	real a2 = a*a;
	real gv = ndotl * sqrt(a2 + (1.0-a2)*ndotv*ndotv);
	real gl = ndotv * sqrt(a2 + (1.0-a2)*ndotl*ndotl);
	return 2.0*ndotv*ndotl / (gv + gl + 1e-7);
}

__device__ inline vec3 ggx_F(const vec3& f0, real vdoth) {
...
__device__ inline vec3 ggx_brdf(const GpuMaterial& mat,
	...
	real ndoth = rmax(dot(n, h), 1e-7);
	real vdoth = rmax(dot(v, h), 1e-7);

	vec3 f0 = vec3(0.04,0.04,0.04)*(1.0-(real)mat.metallic)
			+ mat.albedo*(real)mat.metallic;

	vec3   F = ggx_F(f0, vdoth);
	real D = ggx_D(ndoth, a);
	real G = ggx_G2(ndotv, ndotl, a);

	vec3 specular = F * (D * G / (4.0 * ndotv * ndotl));
	vec3 ms       = ggx_ms_brdf(f0, (real)mat.roughness, ndotv, ndotl);
	vec3 diffuse  = mat.albedo / GPU_PI * (1.0-(real)mat.metallic)
				  * (vec3(1,1,1) - F);
...
__device__ inline vec3 ggx_sample_vndf(const vec3& wo, real a, curandState* rng) {
	...
	real r   = sqrt(rand_double(rng));
	real phi = 2.0 * GPU_PI * rand_double(rng);
	real t   = r * cos(phi);
	real s   = r * sin(phi);
	real bz  = sqrt(rmax(0.0, 1.0 - t*t - s*s));

	vec3 nh = t*t1 + s*t2 + bz*vh;
	return unit_vector(vec3(a*nh.x(), a*nh.y(), rmax(0.0, nh.z())));
}

// The h.wo factor of the VNDF cancels, so it takes the two cosines only. They
// must be formed exactly as the BSDF forms them: at low roughness D is
// delta-like, so a last-bit difference in ndoth moves f/pdf by orders of
// magnitude instead of cancelling.
__device__ inline real ggx_vndf_pdf(real ndotwo, real ndoth, real a) {
	real c = rmax(ndotwo, 1e-7);
	return ggx_D(ndoth, a) * ggx_G1(c, a) / (4.0 * c);
}

// Two lobes: GGX specular + Lambertian. Sampler and pdf must agree, so both
// pick a lobe and report the combined density.
__device__ inline real ggx_spec_prob(const GpuMaterial& mat) {
	vec3 f0 = vec3(0.04,0.04,0.04)*(1.0-(real)mat.metallic)
			+ mat.albedo*(real)mat.metallic;
	real ls = 0.2126*f0.x() + 0.7152*f0.y() + 0.0722*f0.z();
	real ld = (1.0-(real)mat.metallic) * (0.2126*mat.albedo.x()
				+ 0.7152*mat.albedo.y() + 0.0722*mat.albedo.z());
	real t  = ls + ld;
	real p  = (t > 1e-9) ? ls / t : 1.0;
	return rmin(rmax(p, 0.1), 0.9);
}

__device__ inline real ggx_combined_pdf(const GpuMaterial& mat, const vec3& v,
	...
	vec3 h     = unit_vector(v + l);
	real ndoth = rmax(dot(n, h), 1e-7);
	real ps    = ggx_spec_prob(mat);
	return ps * ggx_vndf_pdf(ndotv, ndoth, a) + (1.0 - ps) * ndotl / GPU_PI;
}
...
__device__ inline GpuBSDFSample ggx_sample_lobes(const GpuMaterial& mat,
	...
	real p = ggx_combined_pdf(mat, wo, wi, n);
	if (p <= 0.0) return bs;
...
__device__ inline real rd_schlick(real cos_theta, real eta) {
...
__device__ inline bool rd_eval(const GpuMaterial& mat, const vec3& v,
								...
								vec3& f_out, real& pdf_out) {
	real alpha = (real)mat.roughness * (real)mat.roughness;
	real eta   = front_face ? (1.0 / (real)mat.ir) : (real)mat.ir;

	real ndotv = dot(n, v), ndotl = dot(n, wi);
	if (ndotv <= 1e-7) return false;

	// Both densities are written out against the same D and the same cosines
	// the BSDF used, so the two cancel exactly in f/pdf.
	if (ndotl > 0.0) {
		vec3 h = unit_vector(v + wi);
		if (dot(h, n) < 0.0) h = -h;
		real vdoth = rmin(rmax(dot(v, h), 0.0), 1.0);
		real ndoth = rmax(dot(n, h), 1e-7);
		real F = rd_schlick(vdoth, eta);
		real D = ggx_D(ndoth, alpha);
		real G = ggx_G2(ndotv, rmax(ndotl, 1e-7), alpha);
		f_out = vec3(F,F,F) * (D * G / (4.0 * ndotv * rmax(ndotl, 1e-7)));
		pdf_out = F * D * ggx_G1(ndotv, alpha) / (4.0 * ndotv);
		return true;
	}
	...
	real vdoth = dot(v, h), idoth = -dot(wi, h);
	if (vdoth <= 1e-7 || idoth <= 1e-7) return false;

	real ndoth = rmax(dot(n, h), 1e-7);
	real al    = rmax(-ndotl, 1e-7);
	real F = rd_schlick(rmin(rmax(vdoth,0.0),1.0), eta);
	real D = ggx_D(ndoth, alpha);
	real G = ggx_G2(ndotv, al, alpha);
	real denom = eta * vdoth + idoth;
	if (fabs(denom) < 1e-9) return false;

	f_out = mat.albedo * ((1.0 - F) * D * G * vdoth * idoth
						  / (ndotv * al * denom * denom));
	pdf_out = (1.0 - F) * D * ggx_G1(ndotv, alpha) * vdoth * idoth
			/ (ndotv * denom * denom);
	return true;
}
...
__device__ inline GpuBSDFSample rd_sample(const GpuMaterial& mat, const vec3& v,
	...
	real F = rd_schlick(rmin(rmax(dot(v, h), 0.0), 1.0), eta);
...
__device__ inline real sss_Fdr(real eta) {
	if (eta >= 1.0)
...
__device__ inline vec3 sss_Rd_total(const vec3& alpha, real eta) {
	real fd = sss_Fdr(eta);
	real A  = (1.0 + fd) / rmax(1.0 - fd, 1e-6);
	real out[3];
	for (int c = 0; c < 3; ++c) {
		real ap = (c == 0) ? alpha.x() : (c == 1) ? alpha.y() : alpha.z();
		ap = rmin(rmax(ap, 0.0), 0.999);
		real sq = sqrt(3.0 * (1.0 - ap));
...
__device__ inline vec3 sss_brdf(const GpuMaterial& mat,
	...
	real eta = (real)mat.ir;
	real r0  = (1.0 - eta) / (1.0 + eta); r0 *= r0;
	real T_exit  = 1.0 - (r0 + (1.0 - r0) * pow(1.0 - c, 5.0));
	real T_entry = 1.0 - sss_Fdr(eta);
...
__device__ inline GpuBSDFSample gpu_sample(const GpuMaterial& mat,
	...
	if (mat.type == MatType::LAMBERTIAN) {
		...
		real pdf_val = cosine > 0.0 ? cosine / GPU_PI : 0.0;
	...
	if (mat.type == MatType::METAL) {
		...
		r = unit_vector(r + (real)mat.fuzz * rand_in_unit_sphere(rng));
		return { r, mat.albedo, 1.0, true };
	}

	if (mat.type == MatType::DIELECTRIC) {
		real ratio = rec.front_face ? (1.0/(real)mat.ir) : (real)mat.ir;
		vec3   ud    = unit_vector(wo.direction());
		real cos_t = rmin(dot(-ud, rec.normal), 1.0);
...
__device__ inline real gpu_pdf_dir(const GpuMaterial& mat, const vec3& wo,
									  const vec3& wi, const vec3& n) {
	switch (mat.type) {
		case MatType::LAMBERTIAN:
		case MatType::SSS: {
	...
	switch (mat.type) {
		...
		case MatType::METAL: {
			vec3 wi = unit_vector(reflect(-wo, n)
								  + (real)mat.fuzz * rand_in_unit_sphere(rng));
			real c = dot(wi, n);
			if (c <= 0.0) return bs;
			bs.wi = wi; bs.f = mat.albedo / rmax(fabs(c), 1e-9);
		...
		case MatType::DIELECTRIC: {
			real ratio = rec.front_face ? (1.0/(real)mat.ir) : (real)mat.ir;
			vec3   ud    = -wo;
			real cos_t = rmin(dot(wo, n), 1.0);
			real sin_t = sqrt(rmax(0.0, 1.0 - cos_t*cos_t));
			...
			dir = unit_vector(dir);
			bs.wi = dir;
			bs.f  = vec3(1,1,1) / rmax(fabs(dot(dir, n)), 1e-9);
			bs.pdf = 1.0; bs.is_delta = true;
			return bs;
		}
		default:
