...
struct GpuMaterial {
	MatType type;
	vec3    albedo;     // diffuse / scattering color
	float   fuzz;       // metal fuzz
	float   ir;         // IOR (dielectric + SSS boundary)
	float   roughness;  // GGX roughness
	float   metallic;   // GGX metallic
	float   mfp;        // SSS mean free path (scene units); 0 for non-SSS materials
};
